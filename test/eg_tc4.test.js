...
import { loginUser, setProductQuantity, waitForElement } from '../support/helpers.js';
...
describe("Checkout Test - Close Checkout failed window (REQ-CHECKOUT-CFW)", function () {
  it('signs in, triggers checkout popup and closes it', async function () {
    ...
    try {
      ...
      const checkoutBtn = await waitForElement(driver, By.xpath("//*[contains(translate(text(), 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'), 'checkout') or contains(translate(., 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'), 'checkout')]") );
      ...
      // Assert - Popup should contain minimum order amount message
      expect(popupText).to.include('away from the 40');
      expect(popupText).to.include('KM order min');

      // Close the popup using the provided XPath for the X button
